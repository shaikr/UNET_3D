...
def main(configs_folder, data_folder, results_folder, exp_name_prefix):
    ...
    subjects_vol_results = pd.DataFrame(columns=['EXPERIMENT_NAME', 'TEST_ID', 'SPLIT', 'NSLICES', 'VO_COEFF',
                                                 'DICE_COEFF', 'HAUSDORFF_DISTANCE', 'ASSD', 'RECALL', 'PRECISION'])

    subjects_area_results = pd.DataFrame(columns=['EXPERIMENT_NAME', 'TEST_ID', 'SPLIT', 'ISBLANKS', 'NSLICES',
                                                  'MEAN_VO_COEFF', 'STD_VO_COEFF', 'MEAN_DICE_COEFF', 'STD_DICE_COEFF',
    ...
    for exp in all_rel_exps:
        ...
        for split in splits:
            pred_dir = os.path.join(exp, 'predictions', split)
            ...
            for s_id in sub_ids:
                try:
                    ...
                    for i, pred_path in enumerate(preds):
                        ...
                        subjects_vol_results.loc[cur_results_row_vol] = [os.path.basename(exp), sub_id, split,
                                                                         truth.shape[-1], cur_vo, cur_dice, cur_haus,
                                                                         cur_assd, rec, prec]
                        ...
                        subjects_area_results.loc[cur_results_row_area] = [os.path.basename(exp), sub_id, split, -1,
                                                                           truth.shape[-1], np.mean(vods), np.std(vods),
                                                                           np.mean(dice_coeffs), np.std(dice_coeffs),
                                                                           np.mean(recs), np.mean(precs)]
                        ...
                        subjects_area_results.loc[cur_results_row_area] = [os.path.basename(exp), sub_id, split, 0,
                                                                           len(non_blank_inds),
